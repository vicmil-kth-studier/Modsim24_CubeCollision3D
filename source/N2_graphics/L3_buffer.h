...
class UniformBuffer {
public:
    static void set_4f(std::vector<float> data, Program& program, std::string shader_variable_name) {
        if (data.size() != 4) {
    ...
    static void set_mat4f(glm::mat4 data, Program& program, std::string shader_variable_name) {
        int location = glGetUniformLocation(program.id, shader_variable_name.c_str());
        if (location == -1) {
